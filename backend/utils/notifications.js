...
const notifyNewAnswer = async (question, answer, answerAuthor) => {
  if (
    !question.author ||
    question.author.toString() === answerAuthor._id.toString()
  ) {
    return;
  }
 ...
 */
const notifyAnswerAccepted = async (
  answer,
  questionAuthor,
  questionTitle = null
) => {
  if (
    !answer.author ||
    answer.author.toString() === questionAuthor._id.toString()
  ) {
    return;
  }
...
const notifyUpvote = async (contentType, content, voter, questionId = null) => {
  ...
  return await createNotification({
    ...
    relatedQuestion:
      questionId || (contentType === "question" ? content._id : null),
