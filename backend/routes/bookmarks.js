...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const bookmarks = await Bookmark.find(query)
      .populate('question', 'title description tags category difficulty createdAt views upvotes author')
...
// Create a bookmark
router.post('/', auth, [
  body('questionId').optional({ checkFalsy: true }).isMongoId().withMessage('Invalid question ID'),
  body('answerId').optional({ checkFalsy: true }).isMongoId().withMessage('Invalid answer ID'),
  body('listId').optional({ checkFalsy: true }).isMongoId().withMessage('Invalid list ID'),
  body('notes').optional({ checkFalsy: true }).isLength({ max: 500 }).withMessage('Notes must be less than 500 characters')
], async (req, res) => {
  try {
    ...
    if (cleanListId) {
      const list = await BookmarkList.findOne({ _id: cleanListId, user: req.userId });
      if (!list) {
    ...
    if (existingBookmark) {
      ...
      return res.json({ bookmark: existingBookmark, message: 'Bookmark updated' });
    }
    ...
    if (error.name === 'ValidationError') {
      ...
      return res.status(400).json({ message: validationErrors || error.message });
    }
  ...
  try {
    ...
    if (listId !== undefined) {
      if (listId === null) {
        ...
        const list = await BookmarkList.findOne({ _id: listId, user: req.userId });
        if (!list) {
          return res.status(404).json({ message: 'List not found' });
        }
...
router.get('/lists', auth, async (req, res) => {
  try {
    const lists = await BookmarkList.find({ user: req.userId })
...
router.put('/lists/:id', auth, [
  ...
  try {
    ...
    const { name, description, color } = req.body;
    const list = await BookmarkList.findOne({ _id: req.params.id, user: req.userId });
