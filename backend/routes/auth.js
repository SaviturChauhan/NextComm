  ...
  async (req, res) => {
    try {
      ...
      if (!user.password && (user.authProvider === "google" || user.googleId)) {
        console.log(
          "Login attempt: OAuth user tried to login with password:",
          email
        );
        return res.status(400).json({
          message:
            "This account was created with Google Sign-In. Please use Google Sign-In to access your account.",
          useOAuth: true,
        });
      }
      ...
      try {
        ...
        console.warn(
          "Warning: Could not update last active:",
          updateError.message
        );
  ...
  router.get(
    ...
    async (req, res) => {
      try {
        ...
        if (!req.user) {
          console.error(
            "❌ Error: User not found in request after authentication"
          );
          const frontendUrl =
            process.env.FRONTEND_URL || "http://localhost:3000";
        ...
        try {
          ...
          console.warn(
            "⚠️ Warning: Could not update last active:",
            updateError.message
          );
          // Continue even if this fails
        }
        ...
        res.redirect(
          `${frontendUrl}/login?error=google_auth_failed&details=${errorMessage}`
        );
      }
    }
  );
