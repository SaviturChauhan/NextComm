...
router.delete('/answers/:id', admin, async (req, res) => {
  try {
    ...
    if (answer.author) {
      const updatedUser = await User.findByIdAndUpdate(answer.author, { 
        $inc: { 
          answersGiven: -1,
...
router.post('/answers/:id/accept', admin, [
  ...
  try {
    ...
    res.json({ message: `Answer ${isAccepted ? 'accepted' : 'unaccepted'} successfully by admin` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server error' });
  }
