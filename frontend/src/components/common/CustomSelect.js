...
const CustomSelect = ({
  ...
  const displayValue = selectedOption
    ...
    : value && normalizeOptions.length > 0
    ? value
    : placeholder;

  const handleSelect = (optionValue) => {
    // If option is an object, use its value; otherwise use the optionValue directly
    const selectedOpt = normalizeOptions.find(
      (opt) => opt.value === optionValue || opt.label === optionValue
    );
        ...
        className={`
          ...
          ${
            disabled
              ...
              : "cursor-pointer hover:border-primary dark:hover:border-blue-400"
          }
          ${
            isOpen
