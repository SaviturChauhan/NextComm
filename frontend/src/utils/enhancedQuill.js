...
export const setupFormulaButton = (quill) => {
  ...
  toolbar.addHandler("formula", function () {
    ...
    const formula = prompt(
      "Enter LaTeX formula (e.g., E = mc^2 or \\frac{a}{b}):"
    );
      ...
      handlers: {
        ...
        code: function () {
          ...
          const language = prompt(
            "Enter programming language (python, javascript, matlab, cpp, etc.):",
            "python"
          );
          if (language) {
            ...
            if (code !== null) {
              ...
              this.quill.formatLine(range.index, 1, "code-block", {
                code,
                language: lang,
              });
            }
...
export const processContent = (htmlContent) => {
  ...
  const codeBlocks = processed.match(
